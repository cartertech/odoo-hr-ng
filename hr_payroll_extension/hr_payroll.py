...
class hr_payslip(osv.osv):
    ...
    def _book_holiday_hours(self, cr, uid, contract, presence_policy, ot_policy, attendances,
        ...
        for line in ot_policy.line_ids:
            if line.type == 'holiday':
                ot_hours = self._get_applied_time(hours, line.active_after)
    ...
    def _partial_period_factor(self, cr, uid, payslip, contract, context=None):
        ...
        for line in payslip.worked_days_line_ids:
            if line.code == max_hours_code and line.contract_id.id == contract.id:
    ...
    def get_utilities_dict(self, cr, uid, contract, payslip, context=None):
        ...
        if contract:
            res.update({'PPF': {'amount': self._partial_period_factor(cr, uid, payslip, contract, context=context)}})
        ...
        for c in wdl_contracts:
            wdl_ppf_total += self._partial_period_factor(cr, uid, payslip, c, context=context)
